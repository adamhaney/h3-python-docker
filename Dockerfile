...
ARG BUILD_PACKAGES="wget gcc make cmake curl build-essential apt-utils cmake-curses-gui git unzip doxygen"
...
# H3 Depdendencies
RUN apt-get update && \
    apt-get install -yqq ${BUILD_PACKAGES} libffi-dev libgdal-dev zlib1g-dev libgeos-dev libtool clang-format && \
